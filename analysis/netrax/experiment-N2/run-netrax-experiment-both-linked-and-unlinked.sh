...
# string representing the name of your experiment (e.g. "X") consisting of
# alphanumeric characters (but no spaces, dashes, or underscores),
...
experiment_label="$1" ## must contain only alphanumeric characters. Cannot include spaces, dashes, underscores.
...
iqtree=$experiment_path/experiment-$experiment_label.treefile
outputfile1=$experiment_path/experiment-$experiment_label-netrax-output-linked.txt
outputfile2=$experiment_path/experiment-$experiment_label-netrax-output-unlinked.txt


# Run netrax in linked mode
START=$(date +%s)
mpiexec ./netrax --msa $msa --model $model --seed 42 --output $outputfile1 --average_displayed_tree_variant --brlen linked --start_network $iqtree
END=$(date +%s)
netrax_runtime1=$(( $END - $START ))

echo ""
echo ""
echo "netrax linked mode run complete"
echo "output written to $outputfile1"
echo "RUNTIME: $netrax_runtime1 seconds"
echo ""
echo ""


# Run netrax in unlinked mode
START=$(date +%s)
mpiexec ./netrax --msa $msa --model $model --seed 42 --output $outputfile2 --average_displayed_tree_variant --brlen unlinked --start_network $iqtree
END=$(date +%s)
netrax_runtime2=$(( $END - $START ))

echo ""
echo ""
echo "netrax unlinked mode run complete"
echo "output written to $outputfile2"
echo "RUNTIME: $netrax_runtime2 seconds"
echo ""
echo ""


# Create a readme with documentation and copy this script to the experiment folder
cp "../../run-netrax-experiment-both-linked-and-unlinked.sh" "$experiment_path/run-netrax-experiment-both-linked-and-unlinked.sh"
...
echo -e "\nNetrax runtime (linked): $netrax_runtime1 seconds for linked." >>$experiment_path/readme.md
echo -e "\nNetrax runtime (unlinked): $netrax_runtime2 seconds for unlinked." >>$experiment_path/readme.md
