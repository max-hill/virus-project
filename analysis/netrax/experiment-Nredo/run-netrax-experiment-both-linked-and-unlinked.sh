...
outputfile_unlinked=$experiment_path/netrax-output-unlinked


# Run netrax in linked mode
START=$(date +%s)
mpiexec ./netrax --msa $msa --model $model --seed 42 --output $outputfile_linked --average_displayed_tree_variant --brlen linked --start_network $iqtree
END=$(date +%s)
...
# START=$(date +%s)
# mpiexec ./netrax --msa $msa --model $model --seed 42 --output $outputfile_unlinked --average_displayed_tree_variant --brlen unlinked --start_network $iqtree
# END=$(date +%s)
